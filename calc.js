...
function clearAttribute() {
    // removes 'clicked' effect on operators & raw data
...
digits.forEach((number) => {
    ...
    number.addEventListener('click', () => {
        if (display.textContent.length >= 19) {
            // Do not let display exceed 19 char spaces
        ...
        if (display.textContent.length < 19) {
            ...
            if (equation.addition) {
                ...
                if (
                    ...
                    // for 2 or more plus sign clicks
                    display.textContent = ''
                }
            }
            ...
            if (equation.equal && display.textContent) {
                // Number input resets display right after equal sign operation
                display.textContent = ''
                equation.equal = false
            }

            if (display.textContent === '0') {
                // Update display properly if changing from '0'
...
plus.addEventListener('click', () => {
    ...
    if (
        ...
        // switching operations b4 solving
        equal()
    ...
    if ((equation.first || equation.first === 0) && !equation.addition) {
        // Pressing before second operand input
    ...
    if (
        (equation.first || equation.first === 0) &&
        equation.second &&
        equation.addition
    ) {
        // Pressing >once to perform equal operation
        plus.setAttribute('style', 'background: gray;')
        equal()
        equation.addition = true
        raw.textContent = equation.first + ' + '
...
minus.addEventListener('click', () => {
    ...
    if ((equation.first || equation.first === 0) && !equation.subtraction) {
        // First consecutive minus sign click
